...
本パッケージは Unity WebGL 上の TikTok ミニゲーム向けに YooAsset IFileSystem のアダプタ実装を提供します。

## 機能概要

- TikTok ミニゲーム専用の IFileSystem 実装を提供
- SDK の AssetBundle ダウンロードおよびキャッシュフローと連携
- パッケージバージョンリクエスト、マニフェスト読み込み、アセットバンドルのダウンロードと読み込みに対応
- リモートサービスおよび復号サービスとの接続に対応
...
1. Unity プロジェクトの `Packages/manifest.json` を編集し、以下の scoped registry を追加してください：
   ```json
   {
     "scopedRegistries": [
       {
...
2. `manifest.json` の `dependencies` に直接追加：
   ...
   ```
3. Unity の **Package Manager** で **Git URL** を使用して追加：`https://github.com/gameframex/com.gameframex.unity.tuyoogame.yooasset.tiktok.git`
4. リポジトリを Unity プロジェクトの `Packages` ディレクトリにクローンしてください。自動的に読み込まれます。

## 使用例

スケルトンパッケージ——国際版 TikTok SDK が確定次第実装を追加します。ランタイムコードはまだありません。
...
変更履歴は [Releases](https://github.com/gameframex/com.gameframex.unity.tuyoogame.yooasset.tiktok/releases) をご覧ください。
